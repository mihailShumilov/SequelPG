...
### Fixed
- `statement_timeout` is now set, used, and reset on the *same* pooled connection — previously the `SET` could land on a different connection than the query it was meant to guard — and the reset uses `RESET statement_timeout` so a role/database-level default isn't stomped to 0.
- Rapid filter / sort / pagination clicks no longer race each other: content-page loads cancel the previous in-flight fetch, and a load that finishes after a tab switch can no longer write the old tab's rows into the newly opened one.
- Function metadata and DDL lookups now resolve functions whose schema or name needs quoting (CamelCase, spaces); the `regprocedure` cast input was previously built unquoted and the lookup failed.

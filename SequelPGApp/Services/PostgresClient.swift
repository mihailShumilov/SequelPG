    ...
    private func makeObjectDDLQuery(schema: String, name: String, type: DBObjectType) -> PostgresQuery {
        ...
        case .operator:
            return """
                ...
                """
        case .table:
            // Reconstruct CREATE TABLE from pg_catalog. PostgreSQL has no built-in
            // pg_get_tabledef(), so we compose:
            //   - column definitions (type, default, NOT NULL, generated/identity)
            //   - table-level constraints (PK, UNIQUE, FK, CHECK, EXCLUDE)
            //   - PARTITION BY clause for partitioned parents (relkind 'p')
            //   - secondary CREATE INDEX statements (skip the ones backing constraints)
            //   - table and column comments
            return """
                WITH t AS (
                    SELECT c.oid AS reloid, n.nspname AS schema, c.relname AS name, c.relkind
                    FROM pg_class c
                    JOIN pg_namespace n ON c.relnamespace = n.oid
                ...
                cols AS (
                    SELECT
                        ...
                        CASE WHEN a.attnotnull THEN ' NOT NULL' ELSE '' END AS null_clause,
                        CASE
                        ...
                        END AS def_clause
                    FROM pg_attribute a
                    JOIN t ON a.attrelid = t.reloid
                    LEFT JOIN pg_attrdef d ON d.adrelid = a.attrelid AND d.adnum = a.attnum
                    WHERE a.attnum > 0 AND NOT a.attisdropped
                ),
                column_lines AS (
                    SELECT string_agg('    ' || col_name || ' ' || col_type || def_clause || null_clause,
                                      E',\n' ORDER BY attnum) AS list
                    FROM cols
                ),
                cons AS (
                    SELECT
                        ...
                        CASE con.contype
                            WHEN 'p' THEN 1 WHEN 'u' THEN 2 WHEN 'f' THEN 3
                            WHEN 'c' THEN 4 WHEN 'x' THEN 5 ELSE 6
                        END AS sort_order
                    FROM pg_constraint con
                    JOIN t ON con.conrelid = t.reloid
                    -- PG17+ stores NOT NULL as named constraints (contype 'n').
                    -- They're already covered by the per-column "NOT NULL" we
                    -- emit, so skip them to avoid duplicate CONSTRAINT lines.
                    WHERE con.contype <> 'n'
                ),
                constraint_lines AS (
                    SELECT string_agg('    CONSTRAINT ' || quote_ident(conname) || ' ' || def,
                                      E',\n' ORDER BY sort_order, conname) AS list
                    FROM cons
                ),
                partition_clause AS (
                    SELECT ' PARTITION BY ' || pg_get_partkeydef(t.reloid) AS clause
                    FROM t
                    WHERE t.relkind = 'p' AND pg_get_partkeydef(t.reloid) IS NOT NULL
                ),
                indexes_def AS (
                    SELECT string_agg(pg_get_indexdef(i.indexrelid) || ';',
                                      E'\n' ORDER BY ic.relname) AS list
                    FROM pg_index i
                    JOIN pg_class ic ON ic.oid = i.indexrelid
                    JOIN t ON i.indrelid = t.reloid
                    WHERE NOT EXISTS (
                        SELECT 1 FROM pg_constraint cn WHERE cn.conindid = i.indexrelid
                    )
                ),
                table_comment AS (
                    SELECT 'COMMENT ON TABLE ' || quote_ident(t.schema) || '.' || quote_ident(t.name)
                        || ' IS ' || quote_literal(obj_description(t.reloid, 'pg_class')) || ';' AS line
                    FROM t
                    WHERE obj_description(t.reloid, 'pg_class') IS NOT NULL
                ),
                column_comments AS (
                    SELECT string_agg(
                        'COMMENT ON COLUMN ' || quote_ident(t.schema) || '.' || quote_ident(t.name)
                        || '.' || quote_ident(a.attname) || ' IS '
                        || quote_literal(col_description(a.attrelid, a.attnum)) || ';',
                        E'\n' ORDER BY a.attnum
                    ) AS list
                    FROM pg_attribute a
                    JOIN t ON a.attrelid = t.reloid
                ...
                SELECT 'CREATE TABLE ' || quote_ident(t.schema) || '.' || quote_ident(t.name) || E' (\n'
                    || (SELECT list FROM column_lines)
                    || COALESCE(E',\n' || (SELECT list FROM constraint_lines WHERE list IS NOT NULL), '')
                    || E'\n)'
                    || COALESCE((SELECT clause FROM partition_clause), '')
                    || ';'
                    || COALESCE(E'\n\n' || (SELECT list FROM indexes_def WHERE list IS NOT NULL), '')
                    || COALESCE(E'\n\n' || (SELECT line FROM table_comment), '')
                    || COALESCE(E'\n\n' || (SELECT list FROM column_comments WHERE list IS NOT NULL), '')
                FROM t
                """
        }
    }
